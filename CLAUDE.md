...
Projet de reproduction d'un bug Pulsar (nack + restart simultané = DLQ jamais atteinte).
Trois scénarios A/B/C documentés dans `docs/`.
...
- `mvn test -Dtest=ScenarioATest` — un scénario ciblé (A, B ou C)
- `mvn test` — les trois (~8 min total)
...
- `docs/README.md` — résumé exécutif + tableau A/B/C
- `docs/scenario-{a,b,c}.md` — détail par scénario (bug, code, résultats, pièges)
